...
private:
	...
	inline int checkOccupancy(const Eigen::Vector2d &pos) { return grid_map_->getInflateOccupancy(pos); }
...
public:
	...
	ASTAR_RET AstarSearch(const double step_size, Eigen::Vector2d start_pt, Eigen::Vector2d end_pt);
...
inline bool AStar::Coord2Index(const Eigen::Vector2d &pt, Eigen::Vector2i &idx) const
{
	...
	{
		printf("Ran out of pool, index=%d %d ", idx(0), idx(1));
		return false;
	}
