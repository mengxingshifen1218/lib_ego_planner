...
private:
    int N;
    Eigen::MatrixXd headPVA; // 起点的位置、速度和加速度
    Eigen::MatrixXd tailPVA; // 终点的位置、速度和加速度
    Eigen::VectorXd T1;      // 时间t
    BandedSystem A;          // 系数矩阵
    Eigen::MatrixXd b;       // 是一个瘦子矩阵， 且所存的多项式系数矩阵与多项式类的顺序相反，

    // Temp variables
    Eigen::VectorXd T2; // 时间t的二次方
    Eigen::VectorXd T3; // 时间t的三次方
    Eigen::VectorXd T4; // 时间t的四次方
    Eigen::VectorXd T5; // 时间t的五次方
    Eigen::MatrixXd gdC;
...
public:
                         ...
                         const Eigen::VectorXd &ts)
    {
        if (inPs.cols() == 0) {
            ...
            for (int i = 0; i < N - 1; i++) {
                // 上一段终点的Jerk与下一段Jerk的对应导数系数的关系
                A(6 * i + 3, 6 * i + 3) = 6.0;
                A(6 * i + 3, 6 * i + 4) = 24.0 * T1(i);
                A(6 * i + 3, 6 * i + 5) = 60.0 * T2(i);
                A(6 * i + 3, 6 * i + 9) = -6.0;
                // 上一段终点的Snap与下一段Snap的对应导数系数的关系
                A(6 * i + 4, 6 * i + 4)  = 24.0;
                A(6 * i + 4, 6 * i + 5)  = 120.0 * T1(i);
                A(6 * i + 4, 6 * i + 10) = -24.0;
                // 上一段终点的Pos与给定中间点之间的约束关系
                A(6 * i + 5, 6 * i)     = 1.0;
                A(6 * i + 5, 6 * i + 1) = T1(i);
                A(6 * i + 5, 6 * i + 2) = T2(i);
                A(6 * i + 5, 6 * i + 3) = T3(i);
